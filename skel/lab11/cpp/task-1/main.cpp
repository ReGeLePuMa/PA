...
class DisjointSet
{
 ...
 public:
    ...
    DisjointSet(int nodes) : parent(nodes + 1), size(nodes + 1)
    {
...
struct MSTResult
{
    ...
    vector<pair<int, int>>
        edges;  // muchiile din MST-ul gasit (ordinea nu conteaza)

    MSTResult(int cost, const vector<pair<int, int>>& edges)
        : cost(cost), edges(edges)
    {
    }
};
...
class Task
{
 ...
 private:
    ...
    // muchiile din graf: (node, neigh, w) - muchie de la node la neigh de cost
    // w
    vector<Edge> edges;
    ...
    MSTResult get_result()
    {
        ...
        for (const auto& [node, neigh, w] : edges)
        {
            ...
            {
                disjointset._union(node, neigh);
                cost += w;
                mst.push_back({node, neigh});
            }
        }
        return {cost, mst};
    }
