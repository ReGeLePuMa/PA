...
class Task
{
 ...
 private:
    ...
    Result cmlsc()
    {
        ...
        // dimensiune (dp[i] = ...) nu poate sa stocheze informatia necesara
        // (asemantor cu RUCSAC)
        ...
        //			Obs. Si aici am numit recurenta pornind de la ce ni
        //se cera in enunt!

        // Caz de baza:
        // dp[0][j] = 0 (daca nu luam element din primul vector)
        // dp[i][0] = 0 (daca nu luam element din al doilea vector)
        // P.S. Aceste cazuri au fost tratate prin initializarea lui dp.
        // (Sesizati? :p)
        ...
        for (int i = 1; i <= n; ++i)
        {
            ...
            {
                if (v[i] == w[j])
                {  // (v[i], w[j]) pot extinde cu 1 o solutie care se termina
                   // cel mult pe (i-1, j-1)
                    dp[i][j] = dp[i - 1][j - 1] + 1;
                }
                else
                {  // v[i], w[j] nu pot extinde impreuna o solutie
                    // v[i] poate extinde o solutie, caz in care se cupleaza cu
                    // un w[k], k < j (k = 1...j-1) w[j] poate extinde o
                    // solutie, caz in care se cupleaza cu un v[k], k < i (k
                    // = 1...i-1)
        ...
        {
            if (v[i] == w[j])
            {  // v[i], w[j] a extins candva o solutie
                result.subsequence.push_back(
                    v[i]);  // adaugam elementul v[i] la solutie
