...
class Task
{
 ...
 private:
    ...
    int coing_change()
    {
        ...
        // initial am valori foarte mari in dinamica (presupun ca nu se poate
        // forma suma S) in definirea recurentei am spus practic ce mi se cerea
        // in enunt Sesizati? :p

        // cazul de baza: suma 0 nu necesita monede
        dp[0] = 0;

        // cazul general: suma s; s se poate descompune ca s = s1 + s2... cine
        // sunt s1 si s2? 		O varianta simpla: s1 = o moneda (oarecare) s     = c
        // + (s - c)
        //        1 coin     restul
        // dp[s] =  1     + dp[s-c]
